...
import pdb
def add_lens_blur(orig, radius=5, components=4, gamma=2):
    blur = lens_blur(orig, radius, components, exposure_gamma=gamma)
    # if len(blur.shape) == 2 or blur.shape[2] == 1:
    #     blur = cv2.cvtColor(blur, cv2.COLOR_GRAY2BGR)
    return blur

def add_motion_blur(orig, dis=4, angle=45):
    blur = motion_blur(orig, dis, angle)
    # if len(blur.shape) == 2 or blur.shape[2] == 1:
    #     blur = cv2.cvtColor(blur, cv2.COLOR_GRAY2BGR)
    return blur
...
class RealESRGANDataset(data.Dataset):
    ...
    def __getitem__(self, index: int) -> Dict[str, torch.Tensor]:
        ...
        if random.uniform(0, 1) < 1:
            ...
            dis = np.random.randint(4, 10)
            angle = np.random.randint(0, 180)
            radius = np.random.randint(2, 4)
            components = np.random.randint(1, 3)
            gamma = np.random.randint(1, 3)
            blur = add_motion_blur(img_hq*255, dis, angle)
            blur = add_lens_blur(blur, radius, components, gamma)

            blur = blur / 255.0
            blur = torch.from_numpy(
                blur[..., ::-1].transpose(2, 0, 1).copy()
            ).float()
        ...
        return {
            "hq": img_hq, "hq_blur":blur,"kernel1": kernel, "kernel2": kernel2,
