...
class RealESRGANBatchTransform(BatchTransform):
    ...
    def __call__(self, batch: Dict[str, Union[torch.Tensor, str]]) -> Dict[str, Union[torch.Tensor, List[str]]]:
        ...
        if self.resize_hq and stage2_scale != 1:
            # resize hq
            hq_real = F.interpolate(hq_real, size=(stage2_h, stage2_w), mode="bicubic", antialias=True)
            hq_real = F.interpolate(hq_real, size=(ori_h, ori_w), mode="bicubic", antialias=True)
        self.gt = hq_real
        self.lq = lq
        ...
        hq_real = (self.gt * 2 - 1).float().permute(0, 2, 3, 1).contiguous()
        N = lq.size(0)
        batch['original_size_as_tuple'] = torch.tensor([512, 512]).repeat(N, 1).to(lq.device)
        batch['crop_coords_top_left'] = torch.tensor([0, 0]).repeat(N, 1).to(lq.device)
        batch['target_size_as_tuple'] = torch.tensor([512, 512]).repeat(N, 1).to(lq.device)

        return dict(jpg=hq_real, hint=lq, txt=batch["txt"], original_size_as_tuple=batch['original_size_as_tuple'],\
                    crop_coords_top_left=batch['crop_coords_top_left'], target_size_as_tuple=batch['target_size_as_tuple'])
