...
def load_weight(weight_path: str) -> Dict[str, torch.Tensor]:
    if weight_path.endswith(".safetensors"):
        weight = load_safetensors(weight_path)
        return weight
...
for k,v in sd_weights.items():
    fout.write(f"{k}\t{v.shape}\n")
fout.close()
fout = open("model_old.txt", "w")
for k,v in scratch_weights.items():
    fout.write(f"{k}\t{v.shape}\n")
fout.close()
# assert 0
...
for weight_name in scratch_weights.keys():
    ...
    if target_name in target_model_weights:
        ...
        # else we copy pretrained weight with additional channels initialized to zero
            ...
            print(f"Shape mismatch: {target_name} -> {weight_name}")
