    ...
    def sample_with_mixdiff(
        ...
        for i, step in enumerate(iterator):
            ...
            for hi, hi_end, wi, wi_end in tiles_iterator:
                ...
                batch = {}
                batch['original_size_as_tuple'] = torch.tensor([512, 512]).repeat(b, 1).to(device)
                batch['crop_coords_top_left'] = torch.tensor([0, 0]).repeat(b, 1).to(device)
                batch['target_size_as_tuple'] = torch.tensor([512, 512]).repeat(b, 1).to(device)
                batch['txt'] = [positive_prompt] * b
                ...
                tile_cond = {
                    "c_latent": [self.model.apply_condition_encoder(tile_cond_img)],
                    "c_crossattn": [self.model.get_learned_conditioning(batch)]
                }
                tile_uncond = {
                    "c_latent": [self.model.apply_condition_encoder(tile_cond_img)],
                    "c_crossattn": [self.model.get_learned_conditioning(batch)]
