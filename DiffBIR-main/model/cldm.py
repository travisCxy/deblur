...
class ControlNet(nn.Module):
    def __init__(
        ...
        if legacy:
            # num_heads = 1
            dim_head = ch // num_heads if use_spatial_transformer else num_head_channels
        #cxy change transformer_depth to transformer_depth_middle
...
class ControlLDM(LatentDiffusion):
    ...
    def apply_model(self, x_noisy, t, cond, *args, **kwargs):
        ...
        if cond['c_latent'] is None:
            ...
            hint = torch.cat(cond['c_latent'], 1)
            control = self.control_model(
                x=x_noisy, hint=hint,
                timesteps=t, context=cond_txt
            )
            #length of 13, shape b,320,64,64
    ...
    # def sample_log(self, cond_img, steps):
        ...
        # #)
        # samples = sampler.sample(
        # steps, shape, cond, positive_prompt="", negative_prompt="",
    ...
    def log_images(self, batch, sample_steps=50):
        ...
        # log["text"] = (log_txt_as_img((512, 512), batch[self.cond_stage_key], size=16) + 1) / 2
        #cxy
        log["text"] = (log_txt_as_img((512, 512), batch["txt"], size=16) + 1) / 2
