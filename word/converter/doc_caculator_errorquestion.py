...
label_types = Types()
#'':0.11
class doc_caculator_errorquestion:
    def __init__(self):
    ...
    def process(self, img, meta_data, prefix, work_dir='', num_columns=1):
    ...
    def set_page_global_size(self, meta_data):
        ...
        if self.num_columns==2:
            valid_page_height *= 0.5
            valid_page_width *= 0.5
        elif self.num_columns==3:
            valid_page_height *= 0.35
            valid_page_width *= 0.35
    ...
    def cast_invalid_regions(self, meta_data):
        ...
        self.num_pix_char = 30
        char_pixs = 0
        num_char = 0
        for doc_text in doc_texts:
            if doc_text['text']!='' and '_' not in doc_text['text'] and '`' not in doc_text['text']:
                #print(doc_text['text'])
                for char in doc_text['text']:
                    if char in self.size_map.keys():
                        num_char += self.size_map[char]/0.352
                    else:
                        num_char += 1
                char_pixs += doc_text['bbox'][2]-doc_text['bbox'][0]
        if num_char!=0:
            self.num_pix_char = int(char_pixs/num_char)
            if self.num_columns!=1:
                self.num_pix_char = max(self.num_pix_char, 30)
        ...
        #裁剪包含了pinyin的田字格
        doc_pics = crop_square_cell(doc_pics, doc_texts)
    ...
    def group_paragraphs(self, meta_data):
        ...
        for idx, doc_colum in enumerate(doc_colums):
            ...
            if self.num_columns!=1:
                doc_colum['page_range'][2] = self.valid_page_width - 0.5
            if idx==0:
                bbox[0] = 0
        ...
        for idx, doc_colum in enumerate(doc_colums):
            ...
            for idx, line in enumerate(all_lines):
                ...
                r1 = k1 < 6 and k2 < 3
                # is_para_flag = False
                # if idx<len(all_lines)-1 and is_para(line, all_lines[idx+1], self.is_english_doc):
                #     is_para_flag = True
                r2 = is_same_para_error_question(line, cur_paragraph['lines'][-1], [doc_colum['base_bbox'][0], doc_colum['base_bbox'][2]], self.is_english_doc)
                # r2 = is_same_para_error_question2(line, cur_paragraph['lines'][-1], is_para_flag)
                if r1  and r2 :
                    cur_paragraph['lines'].append(line)
    ...
    def calculate_line_font_size(self, line, is_table=False):
        ...
        #去除重复的……
        if "……" in all_text and not is_table:
            font_size =  12
            for sentence in line['sentences']:
        ...
        ratio_h = real_height / base_height
    ...
    def cal_underline_space(self, orig_img, sentence, cur_font_size, offset_x, offset_y):
        ...
        for i in range(len(hh)):
            ...
            widths.append(ww[i][1] - ww[i][0])
        base_height = self.num_pix_char
        base_width = self.num_pix_char
    ...
    def get_text_width_underline(self, img, sentence, font_size, img_left_margin, img_top_margin):
        ...
        for i, _char in enumerate(text):
            if _char != '_':
                new_text += _char
            elif _char == '_' and f and i>0 and is_alphabet(text[i-1]) and i<len(text)-1 and (is_alphabet(text[i+1]) or text[i+1] in ['0', '1', '2', '3', '4']):
                continue
            elif _char == '_' and uidx <= len(underline_space) - 1:
                new_text += max(7, underline_space[uidx][1]) * '_'
                uidx += 1
            else:
                new_text += '_' * 7
        text = new_text
        ...
        for colum_id, doc_colum in enumerate(doc_colums):
            ...
            for paragraph in doc_colum['paragraphs']:
                ...
                for line in paragraph['lines']:
                    line = self.get_pinyin_text(line, meta_data['doc_info']['doc_pics'], multi_colums=len(doc_colums)>1)
    ...
    def pad_text_with_underline(self, line_text):
        nhas_underline = 0
        math_flag = False
        ...
        if nhas_underline != 0:
            ...
            for i, char_ in enumerate(line_text):
                ...
                if not math_flag and char_ == '_' and (i == 0 or line_text[i - 1] != '_') :
                    j = i+1
                    while j<len(line_text) and line_text[j]=='_':
                        j += 1
                    cur_count = j-i
                    if cur_count == 1:
                        continue
            ...
            for paragraph in doc_colum['paragraphs']:
                for line_idx, line in enumerate(paragraph['lines']):
                    ...
                    if len(line['sentences'])==0:
                        ...
                        # elif '_' in text and '`' in text:
                            # text = self.pad_text_with_underline(text)
                        ...
                        for idx, sentence in enumerate(line['sentences']):
                            ...
                            if idx==0:
                                ...
                                num_space = int(3*(sentence['bbox'][0] - line['sentences'][idx-1]['bbox'][2])//self.num_pix_char)
                                if len(doc_colums)==1:
        ...
        for colum_id, doc_colum in enumerate(doc_colums):
            ...
            for pid, paragraph in enumerate(doc_colum['paragraphs']):
                ...
                for line in paragraph['lines']:
                    ...
                    line['w_ratio'], line['w_space'] = self.cal_line_font_space(line, [line_start, line_end],0.1)
    ...
    def generate_pictures(self, meta_data):
        ...
        for doc_pic in doc_pics:
            ...
            wcm = page_x2 - page_x1
            hcm = page_y2 - page_y1
            doc_pic['pad_page_h'] = hcm
            img_wh_ratio = (real_x2-real_x1)/(real_y2-real_y1)
            page_wh_ratio = wcm/hcm
            if len(doc_colums)>1:
                if page_wh_ratio > img_wh_ratio:
                    if hcm*img_wh_ratio > self.valid_page_width*0.9:
                        wcm = self.valid_page_width*0.9
                ...
                if page_wh_ratio > img_wh_ratio:
                    ...
                    hcm = wcm / img_wh_ratio
            wcm = min(wcm,  0.9*self.valid_page_width)
            hcm = min(hcm, 0.9*self.valid_page_height)
            # warp = 'warpboth'
            warp = 'inline'
            if len(doc_colums) == 1:
                hiou = 0.
            ...
            doc_pic['warp'] = warp
            k = 0.9
            if doc_pic['is_square_cell']:
                k = 0.8
            doc_pic['page_bbox'] = [page_x1, page_y1, page_x1+wcm*k, page_y1+hcm*k]
    ...
    def set_index(self, meta_data, overlap_pix_threshold=10):
        ...
        if len(doc_colums)==0:
            ...
            for float_content in float_contents:
                if float_content['bbox'][0] < colum_left:
                    colum_left = float_content['bbox'][0]
                if float_content['bbox'][2] > colum_right:
                    colum_right = float_content['bbox'][2]
            doc_colum = {}
            doc_colum['bbox'] = [colum_left, 0, colum_right, 0]
            doc_colum['page_range'] = [0, 0, self.valid_page_width,self.valid_page_height]
            place_para = {'type': 'paragraph', 'lines': [],\
                          'bbox': [doc_colum['bbox'][0] - 1, -1, doc_colum['bbox'][2], 0], 'font_size': 4,\
                          'align': 'both', 'is_place_para': 1, \
                          'line_space': 0.04, 'line_space_rule': "exact", "para_space_before": 0,\
                          "page_height_start": 0, "page_height_end": 0, "right_indent": 0., "n_pad_line": 0, 'index':1, 'float_content_id':[]}
            doc_colum['paragraphs'] = [place_para]
            doc_colum['float_contents'] = float_contents
            doc_colums = [doc_colum]

        other_content = doc_text_boxs+doc_pics+doc_tables
        other_content = sorted(other_content, key=lambda x:x['bbox'][1])
        for index, content in enumerate(other_content):
            if 'colum_id' not in content:
                content['colum_id'] = 0
            pic_bbox = content['bbox']
            content['index'] = index
            ...
            for idx, para in enumerate(doc_colum['paragraphs']):
                ...
                if rule:
                    ...
                    if idx!=len(doc_colum['paragraphs'])-1 and doc_colum['paragraphs'][idx+1]['bbox'][1]<pic_bbox[1]:
                        ...
                        real_h = (pic_bbox[1] - doc_colum['paragraphs'][idx + 1]['bbox'][1])/(doc_colum['paragraphs'][idx + 1]['bbox'][3]-doc_colum['paragraphs'][idx + 1]['bbox'][1])*\
                    ...
                    if 'warp' in content.keys() and content['warp'] == 'warpleft':
                        dis_w += 0.5
                    content['position_h_from_paragraph'] = max(0, dis_h)
            ...
            if not f:
                content['in_page'] = 1
        if len(doc_colums)!=0 and len(doc_colums[-1]['paragraphs'])!=0:
            last_para = doc_colums[-1]['paragraphs'][-1]
    ...
    def process_multi_doc_colums(self, meta_data, overlap_pix_threshold=20):
        ...
        for cid, doc_colum in enumerate(doc_colums):
            left_indets = []
            all_first_line_left_indent = []
            colum_bbox = doc_colum['bbox']
            pre_indents1 = [0, 0, 0]
            pre_indents2 = [0, 0, 0]
            for paragraph in doc_colum['paragraphs']:
                for line in paragraph['lines']:
                    min_relative_diff = self.num_pix_char * 100
                    min_relative_diff_idx = -1
                    for ii, pre_bbox_indent in enumerate(pre_indents2):
                        relative_diff = abs(pre_bbox_indent - line['bbox'][0])
                        if relative_diff < min_relative_diff:
                            min_relative_diff = relative_diff
                            min_relative_diff_idx = ii
                    diff = pre_indents1[min_relative_diff_idx] + my_round((line['bbox'][0] - pre_indents2[min_relative_diff_idx])/self.num_pix_char)
                    if self.num_columns!=1:
                        diff = diff * 0.5
                    pre_indents1.pop(0)
                    pre_indents1.append(diff)
                    pre_indents2.pop(0)
                    pre_indents2.append(line['bbox'][0])
                    line['left_indent'] = int(diff)
                    #print(line['left_indent'], line['line_text'])
        # for cid, doc_colum in enumerate(doc_colums):
        #     left_indets = []
        #     all_first_line_left_indent = []
        #     colum_bbox = doc_colum['bbox']
        #     last_num_char = None
        #     last_line_start = colum_bbox[0]
        #     for para in doc_colum['paragraphs']:
        #         for line in para['lines']:
        #             left_num_char = int((line['bbox'][0]-colum_bbox[0])/self.num_pix_char)
        #             if last_num_char!=None:
        #                 rel_left_num_char = (line['bbox'][0]-last_line_start)/self.num_pix_char
        #                 if rel_left_num_char<0.5:
        #                     left_num_char = rel_left_num_char +
        #                 left_num_char = min(left_num_char, rel_left_num_char)
        #             last_num_char = left_num_char
        #             last_line_start = line['bbox'][0]
        #             line['left_indent'] = left_num_char
        #             print(line['left_indent'],line['line_text'])
        for doc_colum in doc_colums:
            ...
            while  content_id < len(contents):
                ...
                bbox = content['bbox']
                type_ = content['type']
                is_square_cell = content['type']=='pic' and content['is_square_cell']
                #田字格不给予inline属性
                if 'is_place_para' in content.keys() or content_id == len(contents) - 1 or bbox[3] < contents[content_id + 1]['bbox'][1] + overlap_pix_threshold or (type_ == 'paragraph' and contents[content_id + 1]['type'] == 'paragraph'):
                    ...
                    while content_id < len(contents) and contents[content_id]['bbox'][1] + overlap_pix_threshold < div_h2 :
                        last_content_is_square_cell = divs[-1]['type']=='pic' and divs[-1]['is_square_cell']
                        cur_content_is_para = contents[content_id]['type']=='paragraph'
                        if last_content_is_square_cell and cur_content_is_para:
                            break
                        content = contents[content_id]
                    ...
                    paras = []
                    fcs = []
                    is_square_cell = False
                    for div in divs:
                        if div['type']=='paragraph':
                            div['float_contents'] = []
                            paras.append(div)
                        else:
                            if 'is_square_cell' in div.keys() and div['is_square_cell']:
                                is_square_cell = True
                            fcs.append(div)
                    ...
                    for uu, fc in enumerate(fcs):
                        fc['keep_para'] = 0
                        fc['create_para'] = 0
                        if len(paras)!=0 and not is_square_cell:
                            ...
                            # if fc['bbox'][2]>=p_range_x2:
                            #     fc['warp'] = 'warpleft'
                            # elif fc['bbox'][2]<p_range_x1:
                            #     fc['warp'] = 'warpright'
                            # else:
                            ...
                            for para in paras:
                                for line in para['lines']:
                                    if bbox_iou(line['bbox'], right_region)!=0:
                                        right_overlap = True
                                    if bbox_iou(line['bbox'], left_region) != 0:
                                        left_overlap = True
                    ...
                    if inline_pic:
                        ...
                        #     fc['in_page'] = 0
                        # divs = [paras[0]] + fcs + paras[1:]
                        #重写段落和图片并排的排版逻辑，图片并不总是跟着第一段而是跟着最近的一段
                        ...
                        for div_idx, div in enumerate(divs):
                            ...
                            if pid>=0 and divs[pid]['type']=='paragraph':
                                ...
                                #ppbox = last_para['bbox']
                                #follow_para = last_para
                                pid = div_idx+1
                            ...
                            pic_bbox = div['bbox']
                            #dis_h = self.trans_to_page_size(pic_bbox[1] -ppbox[1], cal_h=True) + 0.1 + paras[0]['para_space_before']
                            if div_idx<pid:
                                dis_h = self.trans_to_page_size(pic_bbox[1] - ppbox[1], cal_h=True) + 0.1 + follow_para[
                                    'para_space_before']
                            else:
                                dis_h = self.trans_to_page_size(pic_bbox[1] - ppbox[1], cal_h=True) + 0.1 + follow_para[
                                    'para_space_before'] + 1.25 * (len(follow_para['lines']) * 0.39)
                            div['pid'] = pid
                            dis_w = max(0,self.trans_size(pic_bbox[0] - colum_bbox[0], colum_bbox[2] - colum_bbox[0],
                                                        colum_page_range[2] - colum_page_range[0]))
                            if div['warp']=='warpleft':
                                dis_w += 0.5
                            ...
                            div['in_page'] = 0
                        new_divs = []
                        #last_para_fcs = sorted(last_para['float_contents'], key=lambda x:x['bbox'][0])
                        #new_divs.extend(last_para_fcs)
                        # for div in divs:
                        #     if div['type']=='paragraph':
                        #         new_divs.append(div)
                        #         cur_fcs = sorted(div['float_contents'], key=lambda x:x['bbox'][0])
                        #         new_divs.extend(cur_fcs)
                        #cur_fcs = []
                        #last_para = None
            ...
            for cid, content in enumerate(all_blocks):
                    ...
                    if cid!=len(all_blocks)-1 and all_blocks[cid+1]['type']!='paragraph' and all_blocks[cid+1]['warp']=='inline' :
                        ...
                        if diff<content['n_pad_line']*pt2cm(12):#*0.8:
                            ...
                            while  pid!=len(all_blocks)-1 and all_blocks[pid+1]['type']!='paragraph' and all_blocks[pid+1]['warp']=='inline' and 'page_bbox' in all_blocks[pid+1].keys():
                                ...
                                pad_page_h = max(pad_page_h, all_blocks[pid+1]['pad_page_h'])
                                pid += 1
                            #inline_img_h = max(py2s) - min(py1s)
                            inline_img_h = pad_page_h
                ...
                if content['type']!='paragraph' and  'n_pad_line' in content.keys():
