...
class PublayDetector:
    ...
    def detect(self, img, input_size=(960, 960), t=0.25):
        ...
        outputs = self.triton_client.infer_publay_det(img_pad).copy()
        boxes, scores, clses = self.postprocess(outputs, input_size, ratio)
        res = self.get_text(boxes, scores, clses)
        return res
    ...
    def get_text(self, boxes, scores, clses):
        res = []
        if boxes is None:
            return res
        for i in range(boxes.shape[0]):
            cls = clses[i]
            if scores[i]<0.25 or cls!=0:
                continue 
