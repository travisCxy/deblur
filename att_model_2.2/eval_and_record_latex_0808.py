...
def _create_cmd_line(project, model_path, model_index, data_path, gpu, test_cht=False, test_encode=True):
    ...
    if test_encode and not test_cht:
        cmd = cmd + " --test_encode=1"
    if test_cht:
        cmd = cmd + " --test_cht=1"
    return cmd
...
def _list_model_indexes(model_dir):
    ...
    if not os.path.exists(model_dir):
        print("model dir not exists:", model_dir)
        return indexes
    ...
    for name in names:
        if "ckpt-" in name and ".index" in name:
            index = name[5: -6]
            if "764" in index or "201" in index:
                continue
...
if __name__ == "__main__":
    ...
    save_path = "./save_model_latex0808"
    # model_path = "/mnt/server_data/data/seq_chemical/models_equ"
    model_path = "/mnt/server_data2/data/seq_latex/models_equ_latex_0808"
    record_path = "./record_latex_0808.txt"
    gpu = "7"
    project = 'cht'

    # cht_data_path = "/mnt/server_data/data/seq_chemical/tfrecords_mini/val_cht*"
    # data_path = "/mnt/server_data/data/seq_chemical/tfrecords_mini/val_hw*"
    # chemical_data_path = "/mnt/server_data/data/seq_chemical/tfrecords_mini/val_chemical*"
    # math_data_path = "/mnt/server_data/data/seq_chemical/tfrecords_mini/val_math*"

    cht_data_path = "/mnt/server_data2/data/seq_latex/tfrecords_0809/val_cht*"
    data_path = "/mnt/server_data2/data/seq_latex/tfrecords_0809/val_hw*"
    chemical_data_path = "/mnt/server_data2/data/seq_latex/tfrecords_0809/test_chemical*"
    math_data_path = "/mnt/server_data2/data/seq_latex/tfrecords_0809/test_math*"
    ...
    while True:
        ...
        if next_index != -1:
            ...
            cmd = _create_cmd_line(project, model_path, next_index, math_data_path, gpu, test_cht=False)
            print("run cmd: ", cmd)
            status, math_result = subprocess.getstatusoutput(cmd)
            print(math_result)
            ...
            if status == 0:
                ...
                math_current_score, math_result_lines = get_result_lines(math_result)
                result_lines[-1] += cht_result_lines[-1]
                result_lines[-1] += chemical_result_lines[-1]
                result_lines[-1] += math_result_lines[-1]
