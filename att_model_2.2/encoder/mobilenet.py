...
def Net(input_shape=None,
                ...
                classes=1000,
                dropout=0.2,
                regularizer=tf.keras.regularizers.l2(l=0.000004),
                final_endpoint="expand_conv_layer_14",
                **kwargs):
    """Instantiates the MobileNetV3 architecture.
        ...
        # Arguments
            ...
            model_type: MobileNetV3 is defined as two models: large and small.
                These models are targeted at high and low resource use cases respectively.
            minimalistic: In addition to large and small models this module also contains
                so-called minimalistic models, these models have the same per-layer dimensions
                ...
                less efficient on CPU, we find that they are much more performant on GPU/DSP.
            ...
            dropout: fraction of the input units to drop on the last layer
            regularizer: wich regularizer to use on each conv2d layer (in paper was l2(1e-5))
        # Returns
    ...
    if rows < 32 or cols < 32:
        raise ValueError('Input size must be at least 32x32; got `input_shape=' +
                         str(input_shape) + '`')
    if weights == 'imagenet':
        if minimalistic == False and alpha not in [0.75, 1.0] or minimalistic == True and alpha != 1.0:
            raise ValueError('If imagenet weights are being loaded, '
                             'alpha can be one of `0.75`, `1.0` for non minimalistic or `1.0` for minimalistic only.')
    ...
    if model_type == 'small':
        ...
        if final_endpoint == "flat_crnn_feature":
            x = _inverted_res_block(x, 3, 40, alpha, kernel, 1, se_ratio, 'relu', regularizer, 3)
            ...
            x = _inverted_res_block(x, 6, 80, alpha, 3, 1, None, activation, regularizer, 6)
        else:
        ...
        if final_endpoint == "extra_conv_layer_960" or final_endpoint == "expand_conv_layer_14":
            x = _inverted_res_block(x, 6, 160, alpha, kernel, 1, se_ratio, activation, regularizer, 12)
        else:
        ...
        x = layers.SeparableConv2D(44,kernel_size=3, strides=1, 
            depthwise_regularizer=regularizer, pointwise_regularizer=regularizer, padding='same', name="sconv_1")(x)
        x = layers.BatchNormalization(name='bn_1')(x)
        x = tf.keras.activations.swish(x)

        x = layers.SeparableConv2D(88,kernel_size=3, strides=2, 
            depthwise_regularizer=regularizer, pointwise_regularizer=regularizer,  padding='same', name="sconv_2")(x)
        x = layers.BatchNormalization(name='bn_2')(x)
        x = tf.keras.activations.swish(x)

        x = layers.SeparableConv2D(176,kernel_size=3, strides=1, 
            depthwise_regularizer=regularizer, pointwise_regularizer=regularizer,  padding='same', name="sconv_3")(x)
        x = layers.BatchNormalization(name='bn_3')(x)
        x = tf.keras.activations.swish(x)

        x = layers.SeparableConv2D(176,kernel_size=3, strides=2, 
            depthwise_regularizer=regularizer, pointwise_regularizer=regularizer,  padding='same', name="sconv_4")(x)
        x = layers.BatchNormalization(name='bn_4')(x)
        x = tf.keras.activations.swish(x)

        x = layers.SeparableConv2D(353,kernel_size=3, strides=1, 
            depthwise_regularizer=regularizer, pointwise_regularizer=regularizer,  padding='same', name="sconv_5")(x)
        x = layers.BatchNormalization(name='bn_5')(x)
        x = tf.keras.activations.swish(x)

        x = layers.SeparableConv2D(353,kernel_size=3, strides=2, 
            depthwise_regularizer=regularizer, pointwise_regularizer=regularizer,  padding='same', name="sconv_6")(x)
        x = layers.BatchNormalization(name='bn_6')(x)
        x = tf.keras.activations.swish(x)

        x = layers.SeparableConv2D(706,kernel_size=3, strides=2,
            depthwise_regularizer=regularizer, pointwise_regularizer=regularizer,  padding='same', name="sconv_7")(x)
        x = layers.BatchNormalization(name='bn_7')(x)
        x = tf.keras.activations.swish(x)

        x = layers.SeparableConv2D(1004,kernel_size=3, strides=1, 
            depthwise_regularizer=regularizer, pointwise_regularizer=regularizer,  padding='same', name="sconv_8")(x)
    ...
    if final_endpoint == "expand_conv_layer_14" or final_endpoint == "flat_crnn_feature": #"mixed_5d"
        return x
    
    if 'ultrathin' not in model_type:
        ...
        if final_endpoint == "extra_conv_layer_960": #"mixed_5d"
            return x
    
    else:
        if final_endpoint == 'ultra_thin':
            return x

    # AvgPool2D is optimized in TFLite unlike GlobalAveragePooling2D
    x = layers.AvgPool2D(pool_size=(int(x.shape[1]), int(x.shape[2])))(x)
...
def _inverted_res_block(inputs, expansion, alpha, out_ch, kernel_size, stride, se_ratio, activation, regularizer,
    ...
    if se_ratio:
        ...
        y = layers.AvgPool2D(pool_size=(int(x.shape[1]), int(x.shape[2])),
        ...
        x = layers.Multiply(name=prefix + 'squeeze_excite/Mul')([layers.Activation(hard_sigmoid)(y), x])
